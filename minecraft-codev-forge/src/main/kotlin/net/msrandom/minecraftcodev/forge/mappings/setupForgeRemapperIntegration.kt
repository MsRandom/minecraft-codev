...
import net.msrandom.minecraftcodev.remapper.sourceNamespaceVisitor
    ...
    plugins.withType(MinecraftCodevRemapperPlugin::class.java) {
        ...
        remapper.zipMappingsResolution.add { path, fileSystem, _, data ->
            ...
            if (configPath.exists()) {
                ...
                zipFileSystem(mcp.toPath()).use { mcpFs ->
                    val config = data.decorate(mcpFs.base.getPath("config.json").inputStream()).use { MinecraftCodevPlugin.json.decodeFromStream<McpConfig>(it) }
                    val mappings = mcpFs.base.getPath(config.data.mappings)

                    val namespaceCompleter = MappingNsCompleter(data.visitor.tree, mapOf(MinecraftCodevRemapperPlugin.NAMED_MAPPINGS_NAMESPACE to MinecraftCodevForgePlugin.SRG_MAPPINGS_NAMESPACE), true)
                    ...
                    if (!config.official) {
                        val patched = PatchedMinecraftComponentResolvers.getPatchedOutput(
                            ...
                            (data.configuration.resolutionStrategy as ResolutionStrategyInternal).cachePolicy.also(startParameterResolutionOverride::applyToCachePolicy),
                            checksumService,
                            timeProvider,
                            path.toFile(),
                            project,
        ...
        remapper.zipMappingsResolution.add { _, fileSystem, _, data ->
            ...
            data.visitor.withTree(MinecraftCodevForgePlugin.SRG_MAPPINGS_NAMESPACE) { tree ->
                tree.accept(object : ForwardingMappingVisitor(data.visitor.tree) {
                    ...
                    override fun visitNamespaces(srcNamespace: String, dstNamespaces: List<String>) {
                        ...
                        targetNamespace = MinecraftCodevRemapperPlugin.NAMED_MAPPINGS_NAMESPACE.getNamespaceId(srcNamespace, dstNamespaces)
                    }
...
private fun readMcp(path: Path, source: String, data: MappingResolutionData) =
