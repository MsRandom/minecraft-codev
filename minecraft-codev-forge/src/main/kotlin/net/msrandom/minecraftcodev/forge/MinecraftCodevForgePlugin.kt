...
open class MinecraftCodevForgePlugin<T : PluginAware> : Plugin<T> {
    ...
    override fun apply(target: T) = applyPlugin(target, ::applyGradle) {
        ...
        dependencies.registerTransform(ForgeJarTransformer::class.java) {
            it.from.attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, ArtifactTypeDefinition.JAR_TYPE).attribute(MinecraftCodevPlugin.JAR_TRANSFORMED_ATTRIBUTE, false)
            it.to.attribute(ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE, ArtifactTypeDefinition.JAR_TYPE).attribute(MinecraftCodevPlugin.JAR_TRANSFORMED_ATTRIBUTE, true)
        }
