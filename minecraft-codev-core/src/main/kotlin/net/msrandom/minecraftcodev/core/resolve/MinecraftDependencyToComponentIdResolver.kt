...
open class MinecraftDependencyToComponentIdResolver
@Inject
constructor(
    ...
    fun resolveVersion(
        ...
        rejector: VersionSelector?,
        result: BuildableComponentIdResolveResult,
        identifierFactory: (String, String) -> MinecraftComponentIdentifier,
    ) {
            ...
            for (repository in repositories) {
                val versionList =
                    MinecraftMetadataGenerator.getVersionList(
                        MinecraftComponentIdentifier(componentSelector.module, componentSelector.version),
                        null,
                        repository.url,
                        cacheManager,
                        cachePolicy,
                ...
                if (versionList != null) {
                    ...
                    for ((id, version) in versionList.versions) {
                        ...
                        val accepted =
                            if (acceptor is VersionRangeSelector) {
                                VersionRangeSelector(
                                    acceptor.selector,
                                    { x, y -> compareVersions(x.source, y.source) },
                                    versionParser,
                                ).accept(id)
                        ...
                        if (!resolveContext.attributes.isEmpty) {
                            ...
                            val matching =
                                attributesSchema.matcher().isMatching(
                                    attributes,
                                    resolveContext.attributes as AttributeContainerInternal,
                                )
                            if (!matching) {
                                rejections.add(
                                    RejectedByAttributesVersion(
                                        componentIdentifier,
                                        attributesSchema.matcher().describeMatching(
                                            attributes,
                                            resolveContext.attributes as AttributeContainerInternal,
                                        ),
                                    ),
                        ...
                        val selection: ComponentSelectionInternal =
                            object : ComponentSelectionInternal {
            ...
            if (rule.action.inputTypes.isEmpty() == empty) {
                    ...
                    if (null !in inputValues) {
                        try {
                            ...
                            throw InvalidUserCodeException(
                                "There was an error while evaluating a component selection rule for ${selection.candidate.displayName}.",
                                exception,
                            )
                        }
                    }
                ...
                if (selection.isRejected) {
                    LOGGER.info(
                        ...
                        selection.candidate.displayName,
