...
private fun readTiny(data: MappingResolutionData, path: Path) {
    data.decorate(path.inputStream()).bufferedReader().use { reader ->
        ...
        val namespaceCompleter = if (MinecraftCodevFabricPlugin.INTERMEDIARY_MAPPINGS_NAMESPACE in parts) {
            MappingNsCompleter(data.visitor.tree, mapOf(MinecraftCodevRemapperPlugin.NAMED_MAPPINGS_NAMESPACE to MinecraftCodevFabricPlugin.INTERMEDIARY_MAPPINGS_NAMESPACE), true)
        } else {
            data.visitor.tree
        }
...
fun Project.setupFabricRemapperIntegration() {
    plugins.withType(MinecraftCodevRemapperPlugin::class.java) {
        ...
        remapper.extraFileRemappers.add { mappings, fileSystem, sourceNamespace, targetNamespace ->
            ...
            val json = modJson.inputStream().use {
            ...
            val reader = AccessWidenerReader(object : ForwardingVisitor(writer) {
        ...
        remapper.remapClasspathRules.add { sourceNamespace, targetNamespace, version, mappingsConfiguration ->
            if (sourceNamespace == MinecraftCodevFabricPlugin.INTERMEDIARY_MAPPINGS_NAMESPACE || targetNamespace == MinecraftCodevFabricPlugin.INTERMEDIARY_MAPPINGS_NAMESPACE) {
                listOf(
